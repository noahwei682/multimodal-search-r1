...
def compute_data_metrics(batch, use_critic=True, tokenizer=None):
    ...
    # Add 'Search_Ration' and 'Accuracy' metrics for ImSearch/* tasks
    ...
    for idx, response in enumerate(responses_after_first_user_prompt):
        ...
        if 'multi_turn_response_mask' in batch.batch:
            ...
            response = response[_resp_mask == 1]
        response_text = tokenizer.decode(response)
        if "<search><img></search>" in response_text:  # HACK: search pattern
            search_cnt += 1
    search_ratio = search_cnt/total_count
    count_correct = torch.sum(sequence_score > 0.1).item()
    ...
    metrics = {
        # search
        'critic/imsearch/search_ratio': search_ratio,
        'critic/imsearch/answer_acc': answer_acc,
        # score
        'critic/score/mean':
...
class RayPPOTrainer(object):
        ...
        for sample in samples:
            if also_log_ground_truth_and_image_urls:
                row = {
                    ...
                    "score": sample[2],
                    "reward_model": sample[3], # NOTE: strict sequence
                    "image_url": sample[4]
                }
            else:
                row = {
    ...
    def _validate(self):
            ...
            if 'multi_modal_inputs' in test_batch.non_tensor_batch.keys():
                if 'image_urls' in test_batch.non_tensor_batch:
                    test_gen_batch = test_batch.pop(
                        ...
                        non_tensor_batch_keys=['raw_prompt_ids', 'multi_modal_data', 'multi_modal_inputs', 'image_urls'],
                    )
                else:
                    test_gen_batch = test_batch.pop(
                        batch_keys=['input_ids', 'attention_mask', 'position_ids'],
        ...
        for data_source, rewards in data_source_reward.items():
            ...
            # Add Acc Score
            correct_cnt = sum(1 for x in rewards if x > 0.2)  # HACK: design for reward with 0.1*format
            metric_dict[f'val/{data_source}/score'] = correct_cnt/len(rewards)
            if 'imsearch' in data_source:
                search_cnt = 0
                ...
                for idx, response in enumerate(responses_after_first_user_prompt):
                    ...
                    if "<search><img></search>" in response_text:
                        search_cnt += 1
                search_ratio = search_cnt/len(rewards)
                metric_dict[f'val/{data_source}/search_ratio'] = search_ratio
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'multi_modal_inputs' in batch.non_tensor_batch.keys():
                    if 'image_urls' in batch.non_tensor_batch: # for imsearch dataset, the field 'image_urls' is used for potential search actions
                        gen_batch = batch.pop(
                            batch_keys=['input_ids', 'attention_mask', 'position_ids'],
                            non_tensor_batch_keys=['raw_prompt_ids', 'multi_modal_data', 'multi_modal_inputs','image_urls'],
                        )
                ...
                if rollout_logger_dir is not None:
                    ...
                    if self.config.trainer.train_generations_to_log_to_wandb:
                        wandb.log({"train/generations": wandb.Table(dataframe=df)},step=self.global_steps)
                    all_logs = os.path.join(rollout_logger_dir,"rollouts_logger")
                    os.makedirs(all_logs,exist_ok=True)
                    log_rollout_id = 0
                    with open(os.path.join(all_logs,f"step{self.global_steps}"),'w') as f:
                        for reward,vc,url,completion in zip(log_rewards,log_vcs,log_urls,log_completions):
                            f.write(f"rollout {log_rollout_id}, reward {reward:.2f}\n")
                            f.write(f"VC {vc} ImageURL {url}\nCompletions:\n")
                            f.write(completion)
                            f.write("\n\n")
                            log_rollout_id+=1
                    with open(os.path.join(all_logs,f"step{self.global_steps}.pkl"),'wb') as f:
                        pickle.dump(df,f)
                self.global_steps += 1
